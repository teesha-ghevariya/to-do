...
describe('NodeItemComponent', () => {
  ...
  beforeEach(async () => {
    ...
    component.node = { id: 1, content: 'n', parentId: null, position: 0, createdAt: '', updatedAt: '' } as any;
    component.depth = 0;
    component.hasChildren = true;
    fixture.detectChanges();
  });

  it('should render input with content', () => {
    const input: HTMLInputElement = fixture.nativeElement.querySelector('input');
    expect(input.value).toBe('n');
  });

  it('should emit createSibling on Enter', () => {
  ...
  it('should emit createChild on Shift+Enter', () => {
    spyOn(component.createChild, 'emit');
    const input: HTMLInputElement = fixture.nativeElement.querySelector('input');
    input.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', shiftKey: true }));
    expect(component.createChild.emit).toHaveBeenCalled();
  });

  it('should toggle completion on Ctrl+/', () => {
    spyOn(component, 'toggleCompletion');
    const input: HTMLInputElement = fixture.nativeElement.querySelector('input');
    input.dispatchEvent(new KeyboardEvent('keydown', { key: '/', ctrlKey: true }));
    expect(component.toggleCompletion).toHaveBeenCalled();
  });
});
